...
$('#form_name').on('focus', function(){
    ...
    $('#example').DataTable({
        ...
        columns: [
            ...
            { data: 'edit',
                render: function ( data, type, full, meta ) {
                    window.location.replace = `${window.location.origin}/admin/add_class_room.html?id=${full.id} `;
                    let url =`${window.location.origin}/admin/add_class_room.html?id=${full.id}` //Question
